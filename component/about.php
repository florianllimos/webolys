<div class="container-about" id="qui-sommes-nous" aria-labelledby="titre-about">
  <h2 id="titre-about" class="add-section">
    <span class="color"><strong>Webolys</strong></span>, c'est de l'expertise, de l'innovation et de la créativité mises au service de votre projet digital pour transformer vos idées en solutions web performantes et sur-mesure.
  </h2>
  <p class="container-about-text text add-section">
    Spécialiste des CMS comme WordPress et Joomla, je transforme chaque projet en une expérience fluide, optimisée et totalement personnalisée.
  </p>
  <p class="text add-section">
    Que vous ayez besoin d'un site vitrine élégant, d'une boutique en ligne performante ou d'une plateforme entièrement sur-mesure, mon 
    <span class="color">expertise</span> me permet de maîtriser à la perfection les plugins, 
    d’adapter les templates à vos besoins spécifiques.
  </p>
  <p class="text add-section">  
    Chaque détail est soigné pour garantir une navigation fluide, une interface intuitive et un site performant qui répond à vos objectifs, 
    tout en assurant une expérience utilisateur inoubliable.
  </p>  
  <div class="container-info" role="region" aria-labelledby="stats-tech-aria">
    <h3 id="stats-tech-aria" class="sr-only">Quelques chiffres clés sur Webolys</h3>
    <div class="container-info-card add-section" role="group" aria-roledescription="statistique" aria-label="Plus de 1200 heures de développement">
      <div>
        <p>+ 1200</p>
        <p>heures de développement</p>
      </div>
    </div>
    <div class="container-info-card add-section" role="group" aria-roledescription="statistique" aria-label="Lignes de code infinies">
      <div>
        <p>∞</p>
        <p>lignes de code</p>
      </div>
    </div>
    <div class="container-info-card add-section" role="group" aria-roledescription="statistique" aria-label="Réflexion infinie">
      <div>
        <p>∞</p>
        <p>heures de réflexion</p>
      </div>
    </div>
    <div class="container-info-card add-section" role="group" aria-roledescription="statistique" aria-label="Plus de 10 sites réalisés">
      <div>
        <p>+ 10</p>
        <p>sites réalisés</p>
      </div>
    </div>
  </div>
  <div class="container-about-info" role="region" aria-labelledby="a-propos-florian">
    <h3 id="a-propos-florian" class="sr-only">À propos de Florian, fondateur de Webolys</h3>
    <p class="container-about-text text add-section">
      Chez <span class="color"><strong>Webolys</strong></span>, l'humain est au cœur de mes démarche. Bien que nous soyons actuellement une petite équipe composée d'un seul passionné, 
      chaque projet bénéficie de notre expertise, de notre engagement et de notre volonté d’offrir des solutions digitales exceptionnelles. 
      Je prends le temps de comprendre vos besoins et de vous accompagner personnellement à chaque étape, 
      en vous garantissant une expérience de collaboration unique. À l'avenir, nous aspirons à développer l'équipe pour continuer à offrir des services toujours plus performants et innovants.
    </p>
    <div class="test">
      <div class="container-about-info-card add-section" role="group" aria-label="Fonction : CEO et développeur">
        <div>
          <p>CEO & Développeur</p>
        </div>
      </div>
      <div>
        <p class="container-about-info-card-desc add-section">
          Fondateur de <strong><span class="color">Webolys</span></strong>, Florian est un développeur web passionné et créatif, spécialisé dans la création de sites 
          web sur mesure et la gestion de projets numériques. Avec un diplôme en développement web, il met à profit ses 
          compétences en programmation pour offrir des solutions techniques innovantes adaptées aux besoins de chaque 
          client.
        </p>
        <p class="text add-section"> 
          Toujours à l'affût des dernières tendances, Florian est également un expert en intégration de systèmes 
          complexes et en optimisation de la performance des sites. Son approche collaborative et son attention aux détails 
          font de lui un partenaire de confiance pour vos projets digitaux.
        </p>
      </div>
    </div>    
  </div>  
</div>